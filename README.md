...
| **Threats** | On-demand heuristic scan that scores running processes for signs of attack, data theft, persistence or coin-mining — weighing process metadata, command line, network connections, Authenticode signature and autostart entries into a 0–100 risk score with per-rule findings. Read-only: it flags, never kills. |
